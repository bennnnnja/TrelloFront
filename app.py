...
class App(ctk.CTk):
    def __init__(self, width, height):
        ...
        self._create_default_columns(
            ['To Do', 'Currently Doing', 'Done']
        )

    def _create_sidebar_buttons(self):
        add_column_button = self._create_new_button(
            text='Добавить колонку', row=1, col=0, command=self.add_new_column
        )
        ToolTip(
            add_column_button, 'Вставьте новый столбец на доску',
            delay=0, fg="#ffffff", bg="#1c1c1c",
            padx=10, pady=10, font=('Verdana', 10)
        )
        add_card_button = self._create_new_button(
            text='Добавить карточку', row=2, col=0, command=self.add_new_card
        )
        ToolTip(
            add_card_button, 'Вставьте новую карточку в первую колонку',
            delay=0, fg="#ffffff", bg="#1c1c1c",
            padx=10, pady=10, font=('Verdana', 10)
        )
        save_board_button = self._create_new_button(
            text='Сохранить доску', row=3, col=0, command=self.save_board
        )
        ToolTip(
             save_board_button, 'Сохранить текущую доску',
             delay=0, fg="#ffffff", bg="#1c1c1c",
             padx=10, pady=10, font=('Verdana', 10)
        )
        load_board_button = self._create_new_button(
             text='Загрузить доску', row=4, col=0, command=self.load_board
        )
        ToolTip(
             load_board_button, 'Сохранить загруженную доску',
             delay=0, fg="#ffffff", bg="#1c1c1c",
             padx=10, pady=10, font=('Verdana', 10)
         )
        clear_board_button = self._create_new_button(
             text='Создать новую доску', row=5, col=0, command=self.clear_board
         )
        ToolTip(
             clear_board_button, 'Очистить доску',
             delay=0, fg="#ffffff", bg="#1c1c1c",
             padx=10, pady=10, font=('Verdana', 10)
         )
    ...
    def _create_sidebar_frame(self):
        ...
        logo_label = ctk.CTkLabel(
            sidebar_frame, text='FakeTrello', font=ctk.CTkFont(
                size=20, weight='bold'
            )
        )
    ...
    def _create_default_columns(self, default_cols):
        ...
        self.add_new_card(0, 'НАпишите что-нибудь здесь')

    # def _change_dark_mode(self):
    #     ctk.set_appearance_mode(self.check_var.get())

    def add_new_card(self, column=0, text='Новая карточка...'):
    ...
    def add_new_column(self, column_name='', index=-1, color=None):
        if not column_name:
            dialog = ctk.CTkInputDialog(
                text='Напишите название столбца', title='Новый столбец'
            )
    ...
    def load_board(self):
        ...
        load_dialog = CustomChoiceBox(
            ...
            message='Выберете доску для загрузки',
            items=[b.name[:-5] for b in board_list],
            editable=False
        )
